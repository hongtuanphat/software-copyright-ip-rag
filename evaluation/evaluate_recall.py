...
def evaluate_system(
    ...
    results_by_mode: dict[str, dict[str, list[float]]] = {
        "Dense (FAISS)": {f"recall@{k}": [] for k in k_values},
        "Sparse (BM25)": {f"recall@{k}": [] for k in k_values},
        "Hybrid (FAISS+BM25+RRF)": {f"recall@{k}": [] for k in k_values},
    }

    # Lọc các câu hỏi có nhãn ground truth
...
def print_summary_table(summary: dict[str, Any]) -> None:
    ...
    print("=" * 70)
    print(f"{'Phương Pháp Truy Hồi':<26} | {'Recall@1':<8} | {'Recall@3':<8} | {'Recall@5':<8} | {'Recall@10':<8}")
    print("-" * 70)
    ...
    for mode_name, metrics in summary["modes"].items():
        ...
        print(f"{mode_name:<26} | {r1:<8} | {r3:<8} | {r5:<8} | {r10:<8}")
    print("=" * 70)
